---
title: "alchemy"
date: 2026-01-13T13:45:39Z
---

alchemy is a focused working practice for founders, CTOs, and senior leaders
navigating moments where effort is high, responsibility is concentrated, and
progress has stopped compounding.

I bring over twenty years of senior technical and organisational leadership —
spanning architecture, delivery, and system recovery — to help leaders see what
is actually shaping their organisation, and choose deliberately what comes next.

This is not coaching or process optimisation. It is hands-on, judgment-led work
designed to create clarity where timing, structure, and consequence matter.

If you’re in a pivotal moment, we can begin with a single working conversation.

## A practice for leaders at pivotal moments

When complexity accumulates, clarity becomes the constraint.

alchemy is a working practice built around that moment — when capable people are
working hard, the system feels heavier than it should, and progress no longer
compounds on its own.

My work brings the situation into focus. Together, we surface what is actually
happening — real constraints, structural tensions, decisions carrying weight,
and opportunities with genuine leverage.

My work is steady, discerning, and grounded in a deep understanding of technical
systems, organisations, and human dynamics. I engage directly with the substance
of the problem — architecture, strategy, delivery, leadership context, and the
realities of day-to-day execution. I work within the system, shaping what needs
to change.

What typically emerges is:

- Clear priorities
- Settled architectural decisions
- Delivery aligns more closely with intent

Leadership and teams operate from a shared picture, and progress becomes
coherent.

This is the core of alchemy: strategic direction, architectural judgment,
executional shaping, and organisational clarity — brought together through a way
of working that is focused on what moves the system forward.

## When alchemy matters most

alchemy becomes valuable when things feel harder than they should be.

These are inflection points — the organisation remains capable, but something
essential has become obscured. Progress slows despite effort. Too much of the
system lives in a few people’s heads.

Organisations harden around partial understanding over time — decisions get
embedded without ever being made explicitly, and the ability to choose
deliberately is quietly lost.

You may recognise one or more of these:

- The organisation is busy, but progress feels diffuse
- Decisions loop or stall
- Architecture has outpaced shared understanding
- Different parts of the organisation prioritise different things
- Too much technical, strategic, and organisational weight sits with you
- A persistent tension is present, but hard to name
- A significant transition is approaching — scale-up, rebuild, pivot, or
  leadership shift

These moments call for accurate seeing, shared orientation, and movement that
aligns effort with purpose.

## What this practice is

alchemy exists to support leaders.

For over two decades I’ve worked across engineering, architecture, product,
DevOps, data, and organisational leadership — building global platforms,
rescuing failing systems, guiding delivery under pressure, and helping teams
regain trust and coherence.

Across these contexts, my role has been consistent:  
to see how systems have formed, name what matters, and help people move with
confidence.

I help leaders recognise which decisions are shaping the system by default — and
reclaim them as conscious choice.

I work best when invited into spaces where leaders want to be understood, where
teams need alignment, and where an organisation or product is ready for its next
form. My contribution is discernment, judgment, and direction — the kind that
changes how an organisation moves.

alchemy is for founders, CTOs, heads of product, and leadership teams navigating
change: scaling, rebuilding, re-orienting, or preparing for a significant shift.
It serves people who value emotional intelligence alongside engineering depth,
and who want a partner who can hold technical and human complexity at the same
time.

Clients come to alchemy when they need:

- a trusted advisor who sees across architecture, people, and delivery
- strategic clarity that holds the immediate moment and the longer arc
- sound judgment when decisions carry real consequence
- help strengthening or repairing engineering culture
- language for dynamics that are felt but hard to name

My work helps leaders cut through noise, align effort with intent, and move in
ways that feel grounded, coherent, and true to the organisation they are
building.

## How we begin

Every engagement starts with a focused working conversation.

This is a short, high-bandwidth session. We look directly at what is happening:
the pressure points, the decisions waiting to be made, and the complexity
beneath the surface.

Through this conversation, the shape of the system often becomes clearer: what
actually matters, where friction is accumulating, what is holding momentum back,
and what would create real movement.

Leaders often leave this conversation with a clearer understanding of their
organisation — whether that brings clarity, confirmation, or a necessary
challenge.

From there, it’s usually clear whether continuing makes sense. Either way, the
clarity holds.

This is how alchemy begins — through accurate seeing and a grounded sense of
direction.

## How the work unfolds

There’s no fixed model. The work begins by listening — to leaders, teams, and
the system they’re operating within.

Once that shared view is in place, the work moves directly to the points of
highest leverage — architectural decisions, delivery flow, organisational
dynamics, and the choices that will shape what happens next.

That might include:

- shaping architectural or technical direction
- supporting a pivotal leadership decision
- simplifying a complex or fragile system
- working alongside delivery to restore clarity and momentum

At times, the most effective move is a short, bounded technical intervention:
stabilising a system, tuning a database, simplifying a service boundary, or
removing a recurring bottleneck. When deeper or adjacent expertise is required,
I help bring in the right people and ensure their work integrates cleanly with
the wider system.

Engagements are typically focused and time-bounded — often days or a few weeks —
designed to unlock momentum rather than create dependency. The cadence adapts to
what the organisation needs: high-context leadership conversations, embedded
work with a team, or guidance through a critical transition.

What remains constant is the way we work: steady, perceptive, hands-on, and
always oriented toward clarity, alignment, and forward movement.

## Examples of the work

What follows are common patterns that emerge when clarity is brought to complex
situations.

### Strategic

- Creating shared orientation across leadership, teams, and partners
- Setting architectural direction that will endure
- Supporting decisions that meaningfully constrain what comes next

### Organisational

- Establishing working trust across leadership and teams
- Creating shared frames that make collaboration easier
- Surfacing and addressing structural tensions

### Technical

- Architectural simplification where complexity has accumulated
- Performance improvement focused on real constraints
- Removing systemic bottlenecks
- Targeted interventions that unlock movement

## The outcome

The work tends to result in:

- systems become steadier and more resilient
- teams move with shared understanding
- leaders make decisions with clearer judgment
- understanding is less concentrated in individual heads
- organisations operate from a clearer, truer centre

The journey reshapes the people walking it. As clarity deepens, choices evolve.
Sometimes the path holds. Sometimes it changes.

alchemy supports people and organisations in making those choices consciously —
aligned with who they are becoming and what they are stewarding.

## If you’re at a pivotal moment

If you’re carrying complexity, and you can feel where things are misaligned or
heavier than they should be, this work was built for that.

A single focused conversation is enough to tell whether clarity will change how
the organisation moves — or whether the current shape is already settling.

[Get in touch](https://tally.so/r/mZxNae)

## Moments that shaped the practice

This practice has been shaped by over twenty years of senior technical and
organisational leadership across global platforms, crisis recovery, scale-up
architecture, and high-pressure delivery.

### Edinburgh Fringe — system rescue under extreme pressure

Brought in days before the festival opened after the booking platform failed.
Designed and delivered an interim booking system in 72 hours. The system
supported ticket sales through the opening weeks and allowed the organisation to
continue operating.

### megabus.com — scaling a national platform across borders

Architected and scaled the platform from hundreds of daily transactions to tens
of thousands across the UK, US, and Canada. Led architectural redesigns to
support international expansion under sustained growth.

### KFC UK & Ireland — clarity across complex delivery

Joined during a period of digital and data transformation. Clarified delivery
flows, aligned engineering, marketing, and operations, and improved coordination
between leadership and technical teams at scale.

### Startup to scale-up — building teams and systems under load

As CEO and CTO of a digital studio, grew the organisation from 2 to 30+ people
while delivering systems used globally. Led teams through periods of growth and
contraction, delivering complex systems under pressure.
